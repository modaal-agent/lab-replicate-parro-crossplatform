...
**Added 2026-09-19:** §21 runs both builds on the iPhone Duo simulator with Xcode 27.1 beta, and plans how `matched`
follows `native-swift/` there.
...
**Added 2026-09-19:** phase 5's measurements on the iPhone Duo simulator are in §21.
...
**Added 2026-09-19:** phase 5's first results are §21: Xcode 27.1 beta is installed, both builds run on the iPhone Duo
simulator's inner display in landscape, and `matched` is styled after `native-swift/` there. The other poses, the
`.defaultTabBarPlacement(.sidebar)` trial and `ArrangementView` are not part of §21.
...
**Added 2026-09-19:** §21.8 adds the `device` value `duoinner`, the iPhone Duo simulator's inner display in landscape.
...
## 21. Phase 5 on iPhone Duo: both builds on the inner display, and `matched` after `native-swift/` there (added 2026-09-19)

From the user on 2026-09-19, on `168575f` (DISCOVERY.md, entry "iPhone Duo simulator: both builds on the inner
display"): build and run `matched` and `native-swift/` on the iPhone Duo simulator, compare them, take screenshots in new
device folders, "then find a way to make the look and feel of the Capacitor app match the default style of the SwiftUI
app on iPhone Duo", record the path, and commit logical steps.

This section covers the first part of phase 5's row in §6, "both builds on the iOS 27.1 SDK in the iPhone Duo
simulator". The `.defaultTabBarPlacement(.sidebar)` and `ArrangementView` trials of that row are not part of it.

### 21.1 Machine (read 2026-09-19)

- Xcode 27.1 beta (27A9269) at `/Applications/Xcode_27_1_beta.app` is selected, with the iOS 27.1 SDK; Xcode 26.5
  (17F42) is the other install; runtimes iOS 27.0 (24A434) and 27.1 (24A94401). Phase 5's requirement "Xcode 27.1
  beta installed" is met. The iOS 26.5 runtime and the simulators of §10.6 and §19.3 are not on the machine, so
  iPhone 16 and iPad rounds on iOS 26.5 cannot be repeated.
- Node 22.22.3, npm 11.15.0, Swift 6.4, XcodeGen 2.44.1, ImageMagick 7.1.2-15, macOS 26.6.2 (25G83). DISCOVERY.md lists
  each value with the command that read it.
- **Simulator:** `iPhone Duo`, `2BA513E7-59CC-4754-A9F7-E73F027D9619`, iOS 27.1. Two displays at scale 3: outer
  1398 × 2034 px (466 × 678 pt, screen 1) and inner 2007 × 2853 px (669 × 951 pt, screen 3). It boots open, with the
  inner display in landscape: apps run full screen at 951 × 669 pt.
- **Poses.** No `simctl` subcommand, XCTest API or DeviceHub URL changes the pose, and `osascript` has no assistive
  access to press DeviceHub's buttons. `XCUIDevice.shared.orientation = .portrait` leaves the inner display in
  landscape. This section's rounds cover the inner display in landscape only; §21.9 lists the poses not taken.

### 21.2 Both builds as they are, on the inner display in landscape (round r2)

No source changed for this round. Both build with Xcode 27.1 beta against the iOS 27.1 SDK: `native-swift/` in 11.29 s
with the one `appintentsmetadataprocessor` warning of §18.2; `matched` in 15.95 s after a 7.99 s web build, with four
warnings that Xcode 26.6 did not print, three in `@capacitor/keyboard`'s `Keyboard.m:50` and one at
`NativeNavigationPlugin.swift:271`.

- **Tab bar: the same in both.** SwiftUI's `TabView` with `.sidebarAdaptable` and the plugin's `UITabBarController`
  draw one vertical bar of icons at the trailing edge, buttons at x 881 pt, 44 × 58 pt, y 435 to 643 pt, with the Home
  badge; the status bar is in the same trailing column. No code of phase 7 was written for this.
- **What the web view receives** (round p1): viewport 951 × 669 px; `env(safe-area-inset-right)` 84 px, bottom 34 px,
  top and left 0; `screen` 466 × 678, the outer display; the plugin's `--cap-native-navigation-bottom` 669 px;
  `(min-width: 672px)` true, `(min-width: 992px)` false; user agent "iPhone OS 18_7"; Ionic's classes `plt-iphone` and
  `plt-tablet` together.
- **Differences**, each with the cause found:

  | element | `native-swift/` | `matched` | cause in `matched` |
  | --- | --- | --- | --- |
  | list column | flush at x 0–375 pt, white | the floating panel of §19.2 at x 10–385 pt, `#f9f8f8`, from y 88 pt | `matched/index.css:78–95`, taken from iPadOS 26.5; on iPad with iOS 27.0 native also shows a flush column (DISCOVERY.md, round i1) |
  | list rows and header | end 20 pt before the column's edge | keep an end padding that includes 84 px; titles wrap or are cut | Ionic pads `ion-item` and `ion-toolbar` ends by `--ion-safe-area-right`, the 84 px of the trailing bar, which lies beside the detail column |
  | list header, Calendar and Chat | the title from x 20 pt | from x ≈ 83 pt | `watchWindowed()` (`layout.ts:47–55`) sets `windowed`, because `screen` reports the outer display; `Columns.css:33–35` and `matched/index.css:218` add 72 px |
  | list header, Home and Settings | inline, centred title and subtitle in a 58 pt bar | the large title at the leading edge | `matched` renders `LargeTitle` in the list column |
  | rows | no chevrons; selection a grey capsule with black text | chevrons; the selected title in the accent colour | `matched/index.css:120`; `detail` on the rows |
  | "New chat" | x 299–355, y 563–619 pt | x 220–276, y 587–643 pt | the FAB is placed from the trailing safe area and the tab bar's inset |
  | detail title | leading, x 395 pt, y 31.7 pt | centred, x 572–690 pt, y 18 pt | Ionic's iOS `ion-title` is centred |
  | detail content | ends before the tab bar: the composer field x 459–815 pt, y 594 pt | under the tab bar: the last bubble's text to x 925 pt; composer field x 488–835 pt, y 627 pt | no rule gives the detail column the trailing inset |
  | times | "9:41", "13:30–16:00" | "9:41 AM", "1:30 – 4:00 PM" | the simulator's locale is `en_US@rg=nlzzzz`; `Intl.DateTimeFormat(undefined, …)` (`dates.ts:5`) takes `en-US` without the region's 24-hour clock |
  | tab bar after a conversation | stays | gone after Settings is selected, in 9 of 16 runs | §21.3 |

### 21.3 The native tab bar that does not come back

- **Web side.** In the render after a native tab tap, `useColumns()` (`layout.ts:30`), which reads
  `matchMedia('(min-width: 672px)').matches` through `useSyncExternalStore`, returned false once while `innerWidth` read
  951, with no `resize` event. With the path still a conversation, `Tabs` (`App.tsx:127`) sent `setTabbar({ hidden:
  true })`, and in the next render `hidden: false`. The same false render keys the router outlet to `phone` (`App.tsx:138`).
- **Plugin side.** `@capgo/capacitor-native-navigation` 8.3.1 does not show the vertical bar again after a hide: a hide
  and a show 2 s apart left no bar, while its `insets` event reported `tabbarHeight` 669 (round h1).
- On iPhone 16 with iOS 26.5 the same hide and show worked around the conversation (§20.4).

### 21.4 How `matched` follows `native-swift/` on iPhone Duo: options

| option | how | costs |
| --- | --- | --- |
| **A — the native shell sends its traits to the web view** | a `CAPBridgeViewController` subclass in `ios/App/App/` sets classes on `<html>` from `traitCollection`: the idiom and the horizontal size class; CSS keys the iPhone Duo rules to the phone idiom in the column layout, and `layout.ts` takes the columns from the size class | one Swift file (about 60 lines) and its entries in `project.pbxproj`; `SceneDelegate.swift` names the subclass; the web code keeps its media queries for a browser |
| B — web only | key the rules to Ionic's `plt-iphone` with `(min-width: 672px)` | no Swift; the class comes from the user agent; the columns keep the width query and its false read of §21.3; the inner display in portrait, 669 pt, stays below 672 px while native is regular (talk 111461 at 2:46) |

### 21.5 Decisions

- **D18 — Option A.** The shell sends `idiom-phone` or `idiom-pad`, and `size-regular` or `size-compact`, as classes
  on the root element, before the page's scripts run and again on each trait change, with a `nativetraits` event. This
  is step 1 of the strategy in the DISCOVERY.md entry of 2026-09-14, "iPhone Duo: what a Capacitor app can adopt before
  the device ships".
- **D19 — The columns follow the size class when the shell sends one** (`layout.ts`), and `(min-width: 672px)` in a
  browser. This removes the false read of §21.3 on iOS, and gives the inner display in portrait the column layout that
  native shows in regular width.
- **D20 — `windowed` only with the pad idiom.** `watchWindowed()` keeps its comparison, and does nothing with
  `idiom-phone`, since `screen` reports the outer display on iPhone Duo.
- **D21 — The iPhone Duo rules are `matched` CSS under `html.idiom-phone`, in the column layout,** with values read from
  `screenshots/native-swift-duoinner-v1/` and the element frames of round r2. The iPad rules of §19.2 stay as they are:
  no iPadOS 26.5 simulator is left to check them against, and iPadOS 27's flush column (round i1) is a separate change.
- **D22 — Each column takes the inset on its own side.** The list column drops `--ion-safe-area-right`, and the detail
  column ends at `--ion-safe-area-right`, in the column layout for both idioms (talk 111461 at 6:06 and 7:30).
- **D23 — The plugin is not patched.** The fault of §21.3's plugin side stays in `node_modules`; D19 removes the hide
  that the false read caused. A real hide followed by a show on the vertical bar remains open (§21.9).
- **D24 — Times take the region's hour cycle from the shell,** as a `data-hour-cycle` attribute read by `dates.ts`, if
  the steps above leave time for it; otherwise the times stay a gap.

### 21.6 Steps

1. The baseline: round r2's screenshots in `-duoinner-v1` folders, this section and the DISCOVERY.md entry.
2. D18–D20: the Swift subclass, `layout.ts`, and a round on iPhone Duo, iPad with iOS 27.0 and an iPhone on iOS 27.0
   that reads the classes and repeats the Settings step of §21.3.
3. D21 and D22: the CSS, iterated in rounds on the inner display against `native-swift-duoinner-v1`.
4. D24 if taken; the screenshots in `ionic-capacitor-matched-duoinner-v2`; the measurements.

### 21.7 Measures

As §5 and §20.3 for the new files: files and lines added by hand (Swift, TypeScript, CSS), build times, and for each
difference in §21.2 whether it is closed, with the round that shows it.
...
| --- | --- | --- | --- | --- |
| `native-swift-duoinner-v1` | 9: `home`, `home-detail`, `calendar`, `calendar-event`, `chat-empty`, `chat-list`, `chat-group`, `settings`, `settings-detail` | r2 | `168575f` | the commit that adds this section |
| `ionic-capacitor-matched-duoinner-v1` | the same 9; `settings` and `settings-detail` without the tab bar (§21.3) | r2 | `168575f` | the same |

- Xcode 27.1 beta (27A9269), iOS 27.1 (24A94401), macOS 26.6.2 (25G83), Node 22.22.3; the npm versions of §20.3.
- `du -sk`: 3436 and 3628 KiB. No two PNG files under `screenshots/` have the same SHA-256.
- `screenshots/index-duoinner.html` shows the two folders side by side or split (§17.5).
